...
class tensor2skeleton():
    ...
    def __call__(self, tensor, kp_form='openpose_18'):
        ...
        for i in range(coors.shape[1]):
            ...
            if kp_form == 'openpose_25':
                ...
                colors = self.draw_joint_for_vis(self.image_size, coor, LIMB_SEQ_25, 2,
                                                 use_spatial_draw=self.use_spatial_draw)
            elif kp_form == 'openpose_67':
                assert coor.shape[1] == 25 + 2 * 21
                colors = self.draw_joint_for_vis(self.image_size, coor[:, 0:25], LIMB_SEQ_25, 3,
                                                 use_spatial_draw=self.use_spatial_draw)
                colors = self.draw_joint_for_vis(self.image_size, coor[:, 25:46], HAND_SEQ, 3,
                                                 use_spatial_draw=self.use_spatial_draw)
                colors = self.draw_joint_for_vis(self.image_size, coor[:, 46:67], HAND_SEQ, 3,
                                                 use_spatial_draw=self.use_spatial_draw)
            elif kp_form == 'openpose_18':
                assert coor.shape[1] == 18
                colors = self.draw_joint_for_vis(self.image_size, coor[:, 0:18], LIMB_SEQ_18, 3,
                                                 use_spatial_draw=self.use_spatial_draw)
            elif kp_form == 'human36m_17':
                assert coor.shape[1] == 17
                colors = self.draw_joint_for_vis(self.image_size, coor, LIMB_SEQ_HUMAN36M_17, 3,
                                                 use_spatial_draw=self.use_spatial_draw)
            elif kp_form == 'COCO_17':
                assert coor.shape[1] == 17
                colors = self.draw_joint_for_vis(self.image_size, coor, LIMB_SEQ_COCO_17, 3,
                                                 use_spatial_draw=self.use_spatial_draw)
    ...
    def spatial_draw(self, image_size, pose_joints, joint_line_list, radius=2, line_color=[118, 214, 255],
                     cycle_color=[66, 115, 177]):
