...
class TestDataset(BaseDataset):
    ...
    def get_paths(self, opt):
        root = opt.dataroot
        phase = opt.phase
        pairLst = os.path.join(root, 'test-pairs-%s.csv' % phase)  # 'test-pairs-%s.csv' % phase)
        #        pairLst = os.path.join(root, 'arbf_pres.csv')
        name_pairs = self.init_categories(pairLst)

        image_dir = os.path.join(root, '%s' % phase)
        bonesLst = os.path.join(root, 'test-annotation-%s.csv' % phase)  # 'test-annotation-%s.csv' % phase)
